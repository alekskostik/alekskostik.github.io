const STORE_KEY = 'cdt_billing_v27';
...
function makeAccounts(userId, role, yr, seq, inn) {
  ...
  if (role === 'ut') return [
    { accountId:`ACC-${userId}-02`, userId, subAccountTypeId:'02', label:'Счёт услуг', displayNumber:n('02'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
  ...
  if (role === 'ot') return [
    { accountId:`ACC-${userId}-02`, userId, subAccountTypeId:'02', label:'Счёт услуг', displayNumber:n('02'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
    { accountId:`ACC-${userId}-04`, userId, subAccountTypeId:'04', label:'Задатковый счёт ОТ', displayNumber:n('04'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
  ];
  if (role === 'br') return [
    { accountId:`ACC-${userId}-01`, userId, subAccountTypeId:'01', label:'Счёт услуг БР', displayNumber:n('01'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
    { accountId:`ACC-${userId}-03`, userId, subAccountTypeId:'03', label:'Задатковый счёт', displayNumber:n('03'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
  ];
  if (role === 'both') return [
    { accountId:`ACC-${userId}-02`, userId, subAccountTypeId:'02', label:'Счёт услуг', displayNumber:n('02'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
    { accountId:`ACC-${userId}-03`, userId, subAccountTypeId:'03', label:'Задатковый УТ', displayNumber:n('03'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
    { accountId:`ACC-${userId}-04`, userId, subAccountTypeId:'04', label:'Задатковый ОТ', displayNumber:n('04'), balanceFree:0, balanceReserved:0, balanceVirtual:0, isBlocked:false },
  ];
  return [];
}
...
function buildSeed() {
  return {
    ...
    virtualAllocations: [],
    requisites: [
      { requisiteId:'REQ-UT1', accountId:'ACC-USR-UT1-02', bankName:'ПАО «Сбербанк»', bik:'044525225', corAccount:'30101810400000000225', accountNumber:'40702810938000123456', inn:'781234567890', kpp:'780101001', fullName:'ООО «Тест Трейд»', isVerified:true, isDefault:true, createdAt:'2026-03-01T00:00:00Z' },
      { requisiteId:'REQ-UT2', accountId:'ACC-USR-UT2-02', bankName:'АО «Тинькофф»',  bik:'044525974', corAccount:'30101810145250000974', accountNumber:'40702810000001234567', inn:'781234567891', kpp:'780101002', fullName:'ИП Петрова А.С.', isVerified:true, isDefault:true, createdAt:'2026-03-01T00:00:00Z' },
      { requisiteId:'REQ-UT3', accountId:'ACC-USR-UT3-02', bankName:'ПАО «ВТБ»',       bik:'044525187', corAccount:'30101810700000000187', accountNumber:'40702810200000099999', inn:'781234567895', kpp:'780101005', fullName:'ООО «Новиков»',   isVerified:true, isDefault:true, createdAt:'2026-03-01T00:00:00Z' },
      ...
      { requisiteId:'REQ-BO1', accountId:'ACC-USR-BO1-02', bankName:'АО «Альфа-Банк»', bik:'044525593', corAccount:'30101810200000000593', accountNumber:'40702810501234567890', inn:'781234567896', kpp:'780101006', fullName:'ООО «Власов и партнёры»', isVerified:true, isDefault:true, createdAt:'2026-03-01T00:00:00Z' },
    ],
  };
}
...
function loadDemoData(db) {
  ...
  db.transactions = [
    ...
    { txId:'TX-D4', accountId:'ACC-USR-UT1-02', type:'пополнение', status:'завершена', amount:25000,   createdAt:'2026-04-10T10:00:00Z', description:'Пополнение счёта услуг' },
  ];
  saveDB(db);
}
...
function clearAllData(db) {
  ...
  db.requisites=buildSeed().requisites; // сбрасываем реквизиты из seed
  db.accounts.forEach(a=>{ a.balanceFree=0; a.balanceReserved=0; a.balanceVirtual=0; a.isBlocked=false; });
  saveDB(db);
}

function loadDB() {
  try { const raw=localStorage.getItem(STORE_KEY); if(raw){const p=JSON.parse(raw);if(p._v===27)return p;} } catch(e){}
